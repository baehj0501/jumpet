...
export const openYoutubePanel = (options: OpenOptions = {}): void => {
    ...
    // <webview>가 HTML 전체화면(⛶)에 들어갔다 ESC로 나오면 창 표면이 잘못 굳는 경우가 있어
    // (특히 Windows), 전체화면 이탈 시 배경색을 다시 지정하고 크기를 1px 흔들어 컴포지터가
    // 표면을 재할당하게 해 복구한다.
    youtubeWindow.webContents.on('did-attach-webview', (_event, guest) => {
        ...
        guest.on('leave-html-full-screen', repaintSurface)
    })
