...
import { STATE_TICK_INTERVAL_MS, WALK_START_PROBABILITY, WALK_STOP_PROBABILITY } from '../model/constants'
        ...
        const intervalId = setInterval(() => {
            ...
            setState((previousState) => {
                if (previousState === 'idle') {
                    const shouldStartWalking = Math.random() < WALK_START_PROBABILITY
                    if (shouldStartWalking) {
                        return 'walking'
                    }
                    return 'idle'
                }
                const shouldStopWalking = Math.random() < WALK_STOP_PROBABILITY
