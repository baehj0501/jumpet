...
- **할 일 추가** — 텍스트 입력. 최대 40자 (UI input `maxLength` + main reducer `slice` 안전망 둘 다 적용).
- **체크박스로 완료 처리** — **단방향**(진행중 → 완료). 한 번 완료된 것은 다시 진행중으로 못 돌림.
- **개별 삭제** — 진행중 항목만 가능. **완료 항목은 단방향 정책상 삭제 불가** (점수 어뷰징 차단 — 완료 → 삭제 → 재추가 → 재토글로 점수 반복 획득 경로 봉쇄).
- **인라인 텍스트 편집** — 텍스트 클릭으로 수정. 진행·완료 양쪽 가능. 빈 텍스트 저장 시 삭제 (TodoMVC 표준).
- **두 탭**: 진행중 / 완료. v2.0의 '전체' 탭은 폐기. 기본 진입 시 진행중 탭.
- **완료 항목 보관 상한 100개** — 초과 시 createdAt 기준 가장 오래된 완료 항목부터 evict. evict 발생 시 TodoPage 상단에 어떤 항목이 정리됐는지 banner로 잠깐 알림 (자동 사라짐).
...
- **완료 항목 삭제 차단** — × 버튼 미렌더. 점수 어뷰징 경로(완료 → 삭제 → 재추가 → 재토글) 봉쇄.
- **빈 텍스트 add는 무시**. updateText에서 빈 텍스트는 항목 삭제.
- **완료 보관 상한 100개** — 초과 시 FIFO eviction + banner 알림.
- **단방향 정책 적용 범위 정리**: toggle(완료 처리) + 삭제는 단방향. 편집은 양방향(진행·완료 모두 클릭으로 수정 가능).
...
- ~~40자 입력 제한 — 어디서 자르기?~~ → UI maxLength + reducer slice 둘 다 적용.
- ~~`completedAt` 필드 추가?~~ → 도입. Todo 타입에 `completedAt?: number`, toggle 시 `Date.now()` 저장. UI에서 "5/16 14:30" 포맷.
- ~~완료 → 진행중 되돌리기?~~ → 단방향 유지. 실수 클릭은 삭제 후 재추가로만 복구 (단, 완료 항목은 삭제 불가라 사실상 불가능 — 의도된 강한 제약).
- ~~완료 100개 초과 evict 시 UI 알림?~~ → TodoPage 상단 banner로 evict된 항목 텍스트 잠깐 노출 후 자동 사라짐.
